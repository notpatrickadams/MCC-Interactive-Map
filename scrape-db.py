...
def insertData(semester):
    ...
    c.execute(f"create table if not exists '{semester2}' ('CourseName' varchar(255), 'CRN' int, 'Rooms' varchar(255))")
    data = classFind(semester)
    
    #For loop for inserting data
    for course in data:
        for room in course["Rooms"]:
            c.execute(f""" insert into '{semester2}' values (?, ?, ?) """, (course["CourseName"], course["CRN"], room))
